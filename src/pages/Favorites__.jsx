...
const SOURCE_DOMAINS = { CIP:"cip.nl", EO:"eo.nl", "EO/BEAM":"eo.nl", "Reformatorisch Dagblad":"rd.nl", ND:"nd.nl", "NU.nl":"nu.nl", "NPO Radio 1":"nporadio1.nl", YouTube:"youtube.com", NOS:"nos.nl", Trouw:"trouw.nl" };
function normalizeQuotes(s){return (s||"").replace(/[“”„‟]/g,'"').replace(/[’‘‛‹›]/g,"'").replace(/\s+/g," ").trim();}
const dashPattern = ()=>"\\s*[—–-]\\s*";
function ddgUrl(title, source){const dom=SOURCE_DOMAINS[(source||"").trim()]||"";return dom?`https://duckduckgo.com/?q=${encodeURIComponent(title)}+site%3A${encodeURIComponent(dom)}`:`https://duckduckgo.com/?q=${encodeURIComponent(title+" "+(source||""))}`;}
function sectionize(text){const lines=(text||"").replace(/\r/g,"").split(/\n/);const out={news:[],media:[]};let section=null;for(const raw of lines){const line=raw.trim();if(!line)continue;const isHeader=/^(#{0,6}\s*)?(Nieuws|Media)\s*:?\s*$/i.test(line);if(isHeader){section=/Nieuws/i.test(line)?"news":"media";continue;}if(!section)continue;out[section].push(line);}return out;}
function inferType(why,title,source){const s=`${why||""} ${title||""} ${source||""}`.toLowerCase();if(/\b(podcast|radio)\b/.test(s))return"audio";if(/\b(documentaire|video|lezing|stream|worship|clip)\b/.test(s))return"video";return"link";}
function parseNewsLine(line){const L=normalizeQuotes(line);const D=dashPattern();let re=new RegExp(`^"([^"]+)"${D}([^:(]+?)(?:\\s*\\(([^)]+)\\))?:\\s*(.+)$`);let m=L.match(re);if(m){const[,title,source,date,summary]=m;return{title:title.trim(),source:(source||"").trim(),date:(date||"").trim()||undefined,summary:(summary||"").trim()||undefined,url:ddgUrl(title,source)};}re=new RegExp(`^"([^"]+)"${D}([^:]+):\\s*(.+)$`);m=L.match(re);if(m){const[,title,source,summary]=m;return{title:title.trim(),source:(source||"").trim(),summary:(summary||"").trim()||undefined,url:ddgUrl(title,source)};}re=new RegExp(`^(.+?)${D}([^:(]+?)(?:\\s*\\(([^)]+)\\))?:\\s*(.+)$`);m=L.match(re);if(m){const[,title,source,date,summary]=m;return{title:title.trim(),source:(source||"").trim(),date:(date||"").trim()||undefined,summary:(summary||"").trim()||undefined,url:ddgUrl(title,source)};}re=new RegExp(`^(.+?)${D}([^:]+):\\s*(.+)$`);m=L.match(re);if(m){const[,title,source,summary]=m;return{title:title.trim(),source:(source||"").trim(),summary:(summary||"").trim()||undefined,url:ddgUrl(title,source)};}return null;}
function parseMediaLine(line){const L=normalizeQuotes(line);const D=dashPattern();let re=new RegExp(`^"([^"]+)"${D}([^:\\[\\(]+?)(?:\\s*\\[([^\\]]+)\\])?(?:\\s*\\(([^\\)]+)\\))?:\\s*(.+)$`);let m=L.match(re);if(m){const[,title,sourceA,typ,sourceB,why]=m;const source=(sourceB||sourceA||"").trim();const type=(typ||"").trim()||inferType(why,title,source);return{title:title.trim(),source,type,summary:(why||"").trim()||undefined,url:ddgUrl(title,source)};}re=new RegExp(`^(.+?)${D}([^:]+):\\s*(.+)$`);m=L.match(re);if(m){const[,title,source,why]=m;return{title:title.trim(),source:(source||"").trim(),type:inferType(why,title,source),summary:(why||"").trim()||undefined,url:ddgUrl(title,source)};}return null;}
function harvestActueelMediaFromProse(prose){const blocks=sectionize(prose);const cleanup=(l)=>l.replace(/^[-•\d\.\)]\s*/,"").trim();const news=[],media=[];(blocks.news||[]).forEach(raw=>{const line=cleanup(raw);if(!line||/^```/.test(line))return;const it=parseNewsLine(line);if(it)news.push(it);});(blocks.media||[]).forEach(raw=>{const line=cleanup(raw);if(!line||/^```/.test(line))return;const it=parseMediaLine(line);if(it)media.push(it);});return{news,media};}
function mergeUnique(a=[],b=[],keyFn=(x)=>`${(x.title||"").toLowerCase()}__${(x.source||"").toLowerCase()}`){const m=new Map();[...(a||[]),...(b||[])].forEach(it=>{if(!it)return;const k=keyFn(it);if(!k)return;if(!m.has(k))m.set(k,it);});return Array.from(m.values());}

// ---------- CAPTURE HELPERS (cruciaal) ----------

// Wacht tot de chart-surface (canvas of .recharts-wrapper svg) bestaat en "groot genoeg" is
async function waitForChartSurface(rootEl, timeoutMs = 2500) {
  ...
  while (performance.now() - start < timeoutMs) {
    ...
    const bigCanvas = canvases
      ...
      .filter(({ r }) => (r.width || 0) > 160 && (r.height || 0) > 120)
      .sort((a, b) => (b.r.width * b.r.height) - (a.r.width * a.r.height))[0]?.c;
    if (bigCanvas) return bigCanvas;

    // svg (alleen binnen recharts-wrapper en groot genoeg)
    const svg = rootEl.querySelector(".recharts-wrapper svg");
    if (svg) {
      const r = svg.getBoundingClientRect();
      if ((r.width || 0) > 160 && (r.height || 0) > 120) return svg;
      lastFound = svg;
    }

    // even wachten 1 frame + 16ms
    // eslint-disable-next-line no-await-in-loop
    await new Promise((r) => requestAnimationFrame(() => setTimeout(r, 16)));
  }
  return lastFound || rootEl; // als niets, val terug op rootEl
}

// Inline styles in SVG zodat labels/kleuren blijven staan
function inlineComputedStyles(svgEl) {
  ...
  while (walker.nextNode()) {
    ...
    const keys = [
      ...
      "color","opacity","font","font-family","font-size","font-weight","font-style",
      "text-anchor","letter-spacing","word-spacing","dominant-baseline","paint-order","shape-rendering"
    ];
...
async function domSnap(el) {
  const target = el.querySelector(".recharts-wrapper") || el.querySelector("svg") || el;
...
async function captureChartElement(rootEl) {
  ...
  // wacht tot surface beschikbaar is (voorkomt dat we een kleine icoon-SVG pakken)
  const surface = await waitForChartSurface(rootEl, 2500);
  ...
  // 2) SVG (in recharts-wrapper), met inline styles
  if (surface?.tagName?.toLowerCase() === "svg") {
    try {
      const r = surface.getBoundingClientRect();
      if ((r.width || 0) > 160 && (r.height || 0) > 120) {
  ...
  // 3) Fallback DOM-snapshot
  return domSnap(rootEl);
}
...
export default function Favorites() {
  ...
  const registerChartRef = (key) => (el) => {
    ...
    if (el) { el.dataset.chartId = key; chartRefs.current.set(key, el); }
  ...
  async function exportAll(fmt) {
    try {
      ...
      // Sequentieel capturen per DOM-element (nooit de verkeerde SVG/icoon)
      const chartsWithImages = [];
      for (let i = 0; i < (favCharts || []).length; i++) {
        const c = favCharts[i];
        const key = `${c.id}-${i}`;
        const el = chartRefs.current?.get(key) || document.querySelector(`[data-chart-id="${key}"]`);
        // eslint-disable-next-line no-await-in-loop
  ...
  async function generateStream() {
    ...
    try {
      ...
      const label = mode === "bijbelstudie" ? "Bijbelstudie" : mode === "preek" ? "Preek" : mode === "liederen" ? "Liederen" : "Actueel & Media";
      ...
      if ((mode === "actueelmedia" || mode === "liederen") && structured?.type) {
    ...
    <section className="max-w-6xl mx-auto p-3 sm:p-4 space-y-6">
      {showHelp && (
        <div className="rounded-xl border border-sky-200 bg-sky-50 dark:bg-sky-900/30 dark:border-sky-800 p-4 relative shadow-sm">
          <button onClick={dismissHelp} className="absolute top-2 right-2 text-sky-700 dark:text-sky-200 hover:opacity-80" aria-label="Uitleg sluiten" title="Sluiten">×</button>
          <div className="flex items-start gap-3">
            <div className="text-2xl">📚</div>
            <div>
              <h2 className="text-base sm:text-lg font-semibold text-sky-900 dark:text-sky-100 mb-1">Uitleg</h2>
              <p className="text-sm text-sky-900/90 dark:text-sky-100/90">Stel favorieten samen, genereer AI-voorstellen en exporteer alles naar PDF/DOCX.</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="sticky top-0 z-20 -mx-3 sm:-mx-4">
        <div className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 px-3 sm:px-4 py-3 flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <button disabled={busyExport} onClick={() => exportAll("pdf")} className="inline-flex items-center gap-2 text-sm font-medium px-3 py-1.5 rounded-md border border-indigo-600 bg-indigo-600 text-white hover:bg-indigo-700 hover:border-indigo-700 disabled:opacity-60 transition">Export PDF</button>
            <button disabled={busyExport} onClick={() => exportAll("docx")} className="inline-flex items-center gap-2 text-sm font-medium px-3 py-1.5 rounded-md border border-indigo-600 bg-indigo-600 text-white hover:bg-indigo-700 hover:border-indigo-700 disabled:opacity-60 transition">Export DOCX</button>
            <span className="hidden md:inline-block w-px h-5 bg-gray-300 mx-1" aria-hidden="true" />
            <BeginOpnieuwButton label="Begin opnieuw" className="inline-flex items-center gap-2 text-sm font-medium px-3 py-1.5 rounded-md border border-gray-400 bg-gray-300 text-gray-900 hover:bg-gray-400" />
          </div>
        </div>
      </div>
      ...
      {!hasAnyFav ? (
        <div className="max-w-3xl">
          <div className="p-4 rounded-lg bg-yellow-50 border border-yellow-200 text-yellow-800">Je hebt nog geen teksten en/of grafieken geselecteerd. Maak eerst items favoriet.</div>
          <div className="mt-3"><Link to="/" className="text-blue-600 hover:underline">» Ga naar Zoeken</Link></div>
        </div>
      ) : (
        <>
          {/* AI blok */}
          <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
            ...
            <div className="grid sm:grid-cols-4 gap-3 mb-3">
              {[
                ...
                <button key={opt.id} onClick={() => setMode(opt.id)} className={"p-3 rounded-lg border transition " + (mode === opt.id ? "border-indigo-600 bg-indigo-50 dark:bg-indigo-900/30" : "border-gray-300 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700")}>
                  <div className="font-medium">{opt.title}</div>
                </button>
              ))}
            </div>

            <textarea className="w-full p-2 rounded border dark:bg-gray-900 dark:border-gray-700" rows={2} value={extra} onChange={(e) => setExtra(e.target.value)} placeholder="Extra instructies (optioneel)…" />

            <div className="mt-3 flex items-center gap-2">
              <button disabled={aiBusy} onClick={generateStream} className={"px-3 py-1.5 rounded text-sm " + (aiBusy ? "bg-gray-300" : "bg-indigo-600 text-white hover:bg-indigo-700")}>Genereer (stream)</button>
                  ...
                  return (
                    <article key={r.id} className="border rounded-lg p-3 bg-white dark:bg-gray-900">
                      <div className="flex justify-between items-start mb-2">
                        ...
                        <button type="button" className="text-red-500 hover:text-red-600 text-sm" onClick={() => removeAiResult(r.id)}>Verwijderen</button>
                      </div>
                      {isProseOnly ? (r.text ? <AiPretty text={r.text} /> : <p className="text-gray-500">Geen inhoud gevonden.</p>) : (<>{r.structured ? <AiResultCard result={r.structured} /> : <AiPretty text={r.text} />}</>)}
                    </article>
                  );
                })}
              </div>
              ...
              <div className="grid gap-4">
                {favTexts.map((t) => (
                  <div key={t._id || t.ref} className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
                    ...
                    <input className="w-full text-sm p-2 rounded border border-gray-300 dark:border-gray-700 dark:bg-gray-900" value={t.note || ""} onChange={(e) => updateFavTextNote(t._id || t.ref, e.target.value)} placeholder="Notitie bij deze tekst." />
                  </div>
                ))}
              </div>
            )}
              ...
              <div className="grid gap-4">
                {favCharts.map((c, idx) => (
                  <div key={c.id} className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
                    ...
                    {/* BELANGRIJK: wrapper ALLEEN om de grafiek, unieke key, overflow zichtbaar */}
                    <div
                    ...
                    <input className="mt-3 w-full text-sm p-2 rounded border border-gray-300 dark:border-gray-700 dark:bg-gray-900" value={c.note || ""} onChange={(e) => updateFavChartNote(c.id, e.target.value)} placeholder="Notitie bij deze grafiek." />
                  </div>
                ))}
              </div>
            )}
