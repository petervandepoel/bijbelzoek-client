...
function safeJsonParse(maybe) {
  ...
  // Pak JSON uit een codefence ```json ... ```
  const fence = txt.match(/```(?:json)?\s*([\s\S]*?)\s*```/i);
  if (fence) txt = fence[1];
  // Laatste kans: knip van eerste { tot laatste }
  const s = txt.indexOf("{"), e = txt.lastIndexOf("}");
  if (s !== -1 && e !== -1 && e > s) txt = txt.slice(s, e + 1);
  try { return JSON.parse(txt); } catch { return null; }
}

// helper: boeken met meeste treffers voor woorden
...
// Haal bestandsnaam uit Content-Disposition (ondersteunt RFC5987)
...
export default function Favorites() {
  ...
  async function exportAll(fmt /* 'docx' | 'pdf' */) {
    try {
      setBusyExport(true);
      // Werkt of API_BASE wel/niet op /api eindigt
  ...
  async function generateStream() {
    ...
    try {
      ...
      // Probeer JSON uit de stream te vissen (codefence ondersteund)
  ...
  return (
    <section className="max-w-6xl mx-auto p-3 sm:p-4 space-y-6">
      {/* 🔝 Sticky top-bar: titel + export-knoppen blijven altijd zichtbaar */}
      ...
      {/* 🔒 Als er nog geen favorieten zijn */}
      {!hasAnyFav ? (
        ...
        <>
          ...
          <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
            ...
            {aiResults.length === 0 ? (
              <p className="text-gray-500">
                Nog geen AI-resultaten. (<Link to="/" className="text-blue-600 hover:underline">Zoeken »</Link>)
              </p>
            ) : (
          ...
          <div className="space-y-4">
            ...
            {favTexts.length === 0 ? (
              <p className="text-gray-500">
                Nog geen teksten toegevoegd. (<Link to="/" className="text-blue-600 hover:underline">Zoeken »</Link>)
              </p>
            ) : (
              <div className="grid gap-4">
                {favTexts.map((t) => (
          ...
          <div className="space-y-4">
            ...
            {favCharts.length === 0 ? (
              <p className="text-gray-500">
                Nog geen grafieken toegevoegd. (<Link to="/" className="text-blue-600 hover:underline">Zoeken »</Link>)
              </p>
            ) : (
              <div className="grid gap-4">
                {favCharts.map((c) => (
