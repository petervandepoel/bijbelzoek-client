...
// === EXPORT INTEGRATIE (stap 3 – vervanger) ===
const [busyExport, setBusyExport] = useState(false);

// Haal bestandsnaam uit Content-Disposition (ondersteunt ook RFC 5987)
function getDownloadFilename(res, fallback) {
  const cd = res.headers.get("content-disposition") || "";
  // filename*=UTF-8''bestandsnaam.ext
  const star = cd.match(/filename\*\s*=\s*UTF-8''([^;]+)/i);
  if (star) {
    try { return decodeURIComponent(star[1]); } catch { /* noop */ }
  }
  // filename="bestandsnaam.ext"
  const normal = cd.match(/filename="?(.*?)"?(\s*;|$)/i);
  return normal?.[1] || fallback;
}

async function exportAll(fmt /* 'docx' | 'pdf' */) {
  try {
    setBusyExport(true);

    // Zorgt dat het ook werkt als API_BASE geen '/api' eindigt
    const RAW_BASE = (API_BASE || "/api").replace(/\/+$/, "");
    const EXP_API = RAW_BASE.endsWith("/api") ? RAW_BASE : `${RAW_BASE}/api`;
    const url = `${EXP_API}/export/${fmt}`;

    // Gebruikt bestaande state uit useApp():
    // generalNotes, favTexts, favCharts, aiResults, version, searchMode
    const payload = {
      generalNotes,
    ...
    if (!res.ok) {
      // Toon serverfout (bijv. 500) in alert
      const msg = `Export mislukt (${res.status})`;
      throw new Error(msg);
    }
    ...
    const fallbackName = fmt === "pdf"
      ? "Bijbelzoek.nl_Export.pdf"
      : "Bijbelzoek.nl_Export.docx";
  ...
  const actionBtn =
    "inline-flex items-center gap-2 text-sm font-medium px-3 py-1.5 rounded-md border shadow-sm transition " +
    "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500";

  return (
    <section className="max-w-6xl mx-auto p-3 sm:p-4 space-y-6">
      {/* 🔝 Sticky top-bar: titel + altijd zichtbare knoppen */}
   <div className="sticky top-0 z-20 -mx-3 sm:-mx-4">
  <div className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 px-3 sm:px-4 py-3
    ...
    <div className="flex items-center gap-2 flex-wrap">
      ...
      <button
        ...
        className="inline-flex items-center gap-2 text-sm font-medium px-3 py-1.5 rounded-md
                   ...
                   hover:bg-indigo-700 hover:border-indigo-700 disabled:opacity-60 transition"
      ...
      {/* Begin opnieuw / Herstel – SOLIDE kleuren, geen transparantie */}
      <BeginOpnieuwButton
        label="Begin opnieuw"
        className="inline-flex items-center gap-2 text-sm font-medium px-3 py-1.5 rounded-md
                   border border-gray-400 bg-gray-300 text-gray-900
                   hover:bg-gray-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500"
      />
      </div>
  </div>
</div>

      {/* 🔒 Geen favorieten: studie-sectie dicht, maar top-bar blijft zichtbaar */}
      {!hasAnyFav ? (
        <div className="max-w-3xl">
          <div className="p-4 rounded-lg bg-yellow-50 border border-yellow-200 text-yellow-800">
            Je hebt nog geen teksten en/of grafieken geselecteerd. Zorg dat je eerst
            relevante teksten behorend bij jouw thema als favoriet maakt.
          </div>
          <div className="mt-3">
        ...
        <>
          ...
          <div className="space-y-4">
            ...
            {favCharts.length === 0 ? (
              ...
              <div className="grid gap-4">
                {favCharts.length > 0 && favCharts.map((c) => (
