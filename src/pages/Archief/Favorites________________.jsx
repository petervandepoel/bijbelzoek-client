...
function dashPattern() {
  // match em/en/minus dashes surrounded by optional spaces
  return "\\s*[—–-]\\s*";
}
...
function parseNewsLine(line) {
  ...
  // 3) Title — Source (date): summary   (no quotes)
  re = new RegExp(`^(.+?)${D}([^:(]+?)(?:\\s*\\(([^)]+)\\))?:\\s*(.+)$`);
  m = L.match(re);
  if (m) {
    const [, title, source, date, summary] = m;
  ...
  // 4) Title — Source: summary     (no date, no quotes)
  re = new RegExp(`^(.+?)${D}([^:]+):\\s*(.+)$`);
  m = L.match(re);
  if (m) {
    const [, title, source, summary] = m;
...
function parseMediaLine(line) {
  ...
  // 1) "Title" — Source [type] (src?): why/summary
  ...
  // 2) Title — Source: why/summary (no quotes)
  re = new RegExp(`^(.+?)${D}([^:]+):\\s*(.+)$`);
  m = L.match(re);
  if (m) {
...
export default function Favorites() {
  ...
  // --- NIEUW: refs om exacte grafiek-canvas te capturen ---
  ...
  async function exportAll(fmt) {
    try {
      ...
      // Voeg canvas PNG toe aan elke grafiek (exacte weergave)
      const chartsWithImages = (favCharts || []).map((c) => {
        const snap = getChartImageData(c.id);
        return {
          ...c,
            ...
            {favCharts.length === 0 ? (
              ...
              <div className="grid gap-4">
                {favCharts.map((c) => (
                  <div
                    key={c.id}
                    ref={registerChartRef(c.id)}  // <-- belangrijk voor canvas capture
