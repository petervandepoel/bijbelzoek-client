...
export default function Favorites() {
  ...
  // 🔹 Geen favorieten? Studie dichtzetten + melding
  const hasAnyFav = (favTexts?.length || 0) > 0 || (favCharts?.length || 0) > 0;
  if (!hasAnyFav) {
    return (
      <section className="max-w-3xl mx-auto p-4">
        <h1 className="text-2xl font-semibold mb-3">Verrijk, AI &amp; Export</h1>
  ...
  return (
    <section className="max-w-6xl mx-auto p-3 sm:p-4 space-y-6">
      <h1 className="text-2xl font-semibold">Verrijk, AI &amp; Export</h1>

      {/* 🔹 Export + Begin opnieuw / Herstel */}
       {/* 🔹 Export + Begin opnieuw / Herstel */}
      <div className="bg-white dark:bg-gray-800 p-3 rounded-lg shadow flex flex-wrap items-center gap-2">
        <button
          disabled={busyExport}
        ...
        <span className="inline-block w-px h-5 bg-gray-300 mx-2" aria-hidden="true" />

        {/* ✅ Net gestylede Begin opnieuw / Herstel  */}
        {(() => {
          const btnBase =
          ...
          return (
            <>
              <BeginOpnieuwButton
                label="Begin opnieuw"
                className={`${btnBase} bg-gray-100 border-gray-300 text-gray-800 hover:bg-gray-200 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100 dark:hover:bg-gray-600`}
              />
              <BeginOpnieuwButton
                label="Herstel"
                className={`${btnBase} bg-amber-50 border-amber-300 text-amber-800 hover:bg-amber-100 dark:bg-amber-900/30 dark:border-amber-700 dark:text-amber-200 dark:hover:bg-amber-900/40`}
              />
            </>
          );
        })()}
