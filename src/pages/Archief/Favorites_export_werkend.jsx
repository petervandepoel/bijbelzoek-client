...
export default function Favorites() {
  // Collect DOM refs for each chart card to capture canvas as PNG (exact look)
  ...
  function getChartImageData(id) {
    ...
    if (!root) return null;
    // find the first canvas within this chart card
  ...
  // --- veiligere export + exacte grafiek-captures ---
async function exportAll(fmt) {
  try {
    ...
    // Neem bestaande grafieken over en voeg live-canvas PNG toe (exacte look & feel)
    const chartsWithImages = (favCharts || []).map((c) => {
      const snap = getChartImageData(c.id); // gebruikt registerChartRef/getChartImageData
      return {
        ...c,
        imageData: snap?.dataUrl || null, // data URL van canvas
        docxWidth: 640,
        docxHeight: 360,
      };
    });
        ...
        <>
          ...
          <div className="space-y-4">
            ...
            {favCharts.length === 0 ? (
              ...
              <div className="grid gap-4">
                {favCharts.map((c) => (
                  <div key={c.id} ref={registerChartRef(c.id)} className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
                    <div className="flex justify-between items-start mb-2">
                      <div>
